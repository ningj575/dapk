...
export function SupportFloating() {
  ...
  const openWeChatQrPreview = () => {
    if (!isWeChatBrowser) {
      window.location.href = "weixin://";
      return;
    }
    const absoluteQrCode = /^https?:\/\//i.test(qrCode) ? qrCode : new URL(qrCode, window.location.origin).toString();
    const bridge = (window as unknown as { WeixinJSBridge?: { invoke?: (name: string, payload: Record<string, unknown>) => void } }).WeixinJSBridge;
    if (bridge?.invoke) {
      bridge.invoke("imagePreview", { current: absoluteQrCode, urls: [absoluteQrCode] });
      return;
    }
    window.open(absoluteQrCode, "_blank", "noopener,noreferrer");
  };
      ...
      {mode === "wechat" && (
        <div data-testid="wechat-modal" className="fixed inset-0 z-[100] flex items-center justify-center bg-[#101827]/45 px-4 py-8 backdrop-blur-sm" role="dialog" aria-modal="true">
          <div className="relative w-full max-w-[372px] overflow-hidden rounded-[28px] border border-white/70 bg-[#fbfaf7] px-7 pb-8 pt-9 text-center text-[#101827] shadow-[0_30px_90px_-35px_rgba(16,24,39,0.55)]">
            ...
            <button className="mt-5 inline-flex h-12 w-full items-center justify-center gap-2 rounded-[18px] border border-[#171d2a] bg-[#101827] text-base font-black text-white shadow-[0_18px_40px_-16px_rgba(16,24,39,0.42)] transition hover:-translate-y-px hover:bg-[#151f31]" type="button" onClick={openWeChatQrPreview}>
