...
function ModeEditor({
  ...
  return (
    <div className="space-y-4">
      ...
      {mode === "one-click-2" && (
        <textarea className="studio-input resize-none py-4 leading-6" style={{ minHeight: 124 }} maxLength={500} placeholder={videoPromptPlaceholder(mode)} value={prompt} onChange={(event) => setPrompt(event.target.value)} />
      )}
      {mode !== "one-click-2" && (
        <label className="block">
          <span className="mb-2 block text-xs font-bold text-[#7d8492]">提示词</span>
          <textarea className="studio-input resize-none py-4 leading-6" style={{ minHeight: mode === "first-last-2" ? 192 : 96 }} maxLength={500} placeholder={videoPromptPlaceholder(mode)} value={prompt} onChange={(event) => setPrompt(event.target.value)} />
        </label>
      )}
